...
- 启用顶部 `My Notes`、`Comments`、`Similar` 标签页。
- `My Notes` 读取当前文献的 Zotero 子笔记。
- `Comments` 汇总当前文献附件中的划线批注和批注评论。
- `Similar` 基于本地库标题、标签和作者重合度推荐相关文献。
...
- Enables the top `My Notes`, `Comments`, and `Similar` tabs.
- `My Notes` reads Zotero child notes for the current paper.
- `Comments` summarizes highlight annotations and annotation comments from current paper attachments.
- `Similar` recommends local library papers by overlapping title terms, tags, and authors.
